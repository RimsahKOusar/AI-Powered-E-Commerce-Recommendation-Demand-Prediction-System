...
def upgrade() -> None:
    ...
    for ext in ("citext", "pg_trgm", "unaccent", "vector"):
        op.execute(f"CREATE EXTENSION IF NOT EXISTS {ext}")

    sa.Enum("customer", "admin", name="user_role").create(op.get_bind(), checkfirst=True)
    role = sa.Enum("customer", "admin", name="user_role", create_type=False)
